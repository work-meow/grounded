...
- **История чата сжимается, а не обрезается**: последние 20 сообщений уходят
  ...
  него.
- **Запрос перед индексом чистится от синтаксиса BM25.** Строку, которую
  парсер Tantivy не разбирает, он не отвергает — он паникует и роняет индексер
  целиком. Теряется вопросительный знак и дефис внутри слова.
