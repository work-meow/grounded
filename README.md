...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/тестов-618-informational" alt="618 тестов">
...
**Мерка построена на живых вопросах, а не на выдуманных.** Набор из 13
вопросов по пяти демо-документам дошёл до 10/10 и 13/13 и перестал что-либо
показывать. Теперь набор собирается из вопросов, которые действительно
задавали, а к полноте и точности добавилось третье число — обоснованность:
встречается ли каждый факт ответа в тех фрагментах, по которым он написан.
Именно этим измерены два последних изменения, и одно из них не прошло:

| | было | контекстные чанки | расширение запроса |
|---|---|---|---|
| необоснованных утверждений | 25 | **11** | 28 |
| обоснованных ответов | 10/15 | **12/15** | 9/15 |
| самый долгий ответ | 14.3 с | 14.3 с | 16.8 с |
...
- **История чата сжимается, а не обрезается**: последние 20 сообщений уходят
  ...
  него.
- **Запрос перед индексом проходит белый список символов.** Строку, которую
  разбор выражений Pathway не понимает, он не отвергает — он паникует и роняет
  индексер целиком, для всех. Виновата обратная кавычка; теряется
  вопросительный знак и дефис внутри слова.
